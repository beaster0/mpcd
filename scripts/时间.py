...
这个文件就是“我已经跑完零流，现在要算正式任务所需时间尺度”时运行的入口。

可以改什么：
- 一般只改命令行参数，不直接改代码。
- `--no-figures`：只写表，不画图。
- `--figdir <路径>`：改变时间尺度诊断图输出目录。
- `--pipe-axis z`：管道轴向，当前固定为 z。

输入：
- `config/base.json`：用于知道有哪些结构、seed 和管半径。
- `results/ts_r36_g<n>_s<seed>/timeseries.npz`：零流轨迹数据。

输出：
- `tables/analysis/timescales.csv`：每个结构最终采用的时间尺度。
- `tables/analysis/timescale_runs.csv`：每条零流轨迹的详细诊断。
- `figures/timescales/`：时间尺度诊断图。

运行：
- `/home/zhangxh/students/sjl/miniconda3/envs/A/bin/python scripts/时间.py`

时间尺度定义：
- `tau_shape_used`：非球形度自相关时间，用于形状弛豫判断。
- `tau_int_r_used`：质心径向位置自相关时间，用于径向记忆判断。
- `tau_r_used`：保留兼容字段，当前等同于 `tau_int_r_used`。
- `tau_rad_diagnostic`：`R^2 / D_perp`，只作为横向扩散诊断，不直接参与正式步数设计。
...
def iter_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    """列出 时间.py 需要读取的零流轨迹结果。"""
    # cases 保存所有期望存在的零流结果。
    cases: list[dict[str, Any]] = []
    # 管半径用于拼接 run_id，例如 r36。
    radius = float(config["pipe"]["radius"])
    # pipe_tag 是结果目录名的一部分。
    pipe_tag = f"r{radius:g}"
    # n_values 控制 g1/g2/g3/g4 等结构。
    for n in config["structure"]["n_values"]:
        # timescale.seeds 控制每个结构的零流重复数。
        for seed in config["timescale"]["seeds"]:
            # structure 是结构名。
            structure = f"g{n}"
            # run_id 必须和 轨迹.py 的 time 阶段保持一致。
            cases.append({
                "run_id": f"ts_{pipe_tag}_{structure}_s{seed}",
                "structure": structure,
                "seed": seed,
            })
    # 返回所有要分析的零流结果。

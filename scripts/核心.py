...
这个文件不作为日常入口运行。你平时应该运行：
- `scripts/轨迹.py`：获得某一条轨迹数据。
- `scripts/时间.py`：从零流轨迹计算时间尺度并画图。

可以改什么：
- 普通物理参数优先改 `config/base.json`，不要在这里硬编码。
- 只有当你要改真实模拟实现时，才改这里，例如势函数、壁面、MPCD 碰撞、采样内容、进度条显示。
- 新写运行脚本时，应调用这里的 `run_task()`，这样自动继承进度条、输出格式和状态文件。

输入：
- `config/base.json`：物理参数、结构列表、流强、随机种子和服务器设置。
- `data/structures/*.json`：由 `scripts/结构.py` 生成的整体三维网格凝胶结构。
- `轨迹.py` 传入的一条 case 字典。
...
- `results/<run_id>/profiles.npz`：溶剂速度、密度等径向剖面。
"""
...
def print_progress(run_id: str, done: int, total: int, start_time: float, force: bool = False) -> None:
    ...
    # force=False 时每 1% 或完成时打印一次，避免后台日志过大。
    if not force and done < total:
        # last_units 记录每个 run_id 上一次打印到哪个百分点。
        last_units = getattr(print_progress, "_last_units", {})
        # percent_unit 是当前完成的百分点整数。
        percent_unit = int((100.0 * done) / total)
        # 如果当前百分点没有超过上一次打印值，就不刷屏。
        if percent_unit <= last_units.get(run_id, -1):
            return
        # 记录这次已经打印过的百分点。
        last_units[run_id] = percent_unit
        # 把记录挂在函数对象上，避免引入全局变量。
        setattr(print_progress, "_last_units", last_units)
...
def analysis_radius(config: dict[str, Any]) -> float:
    """返回论文和数据分析使用的管半径。

    这个半径代表物理问题里的管道半径 R。速度剖面、径向位置、
    凝胶受限程度都用它归一化。
    """
    pipe = config["pipe"]
...
def geometry_radius(config: dict[str, Any]) -> float:
    """返回 HOOMD 边界实际使用的几何半径。

    MPCD 的近壁层会让有效无滑移位置和几何边界略有偏差。
    因此保留一个 geometry_radius_offset，由空管泊肃叶校准确定。
    """
    return analysis_radius(config) + float(config["pipe"].get("geometry_radius_offset", 0.0))
...
def solvent_count(config: dict[str, Any]) -> int:
    ...
    # 溶剂数按几何半径生成，因为 HOOMD 边界实际限制的是 geometry_radius。
    count = int(round(density * pipe_volume(geometry_radius(config), float(pipe["length"]))))
...
def force_vector(force: float) -> tuple[float, float, float]:
    """把真实体力 g 写成 HOOMD 需要的三维力向量。"""
    # 零流强时不施加体力。
    if force <= 0.0:
        return (0.0, 0.0, 0.0)
    # 体力沿 z 方向施加；x/y 方向为 0。
    return (0.0, 0.0, float(force))
...
def make_snapshot(root: Path, config: dict[str, Any], task: dict[str, str], n_solvent: int) -> Any:
    ...
    radius = geometry_radius(config)
    length = float(pipe["length"])
    # 盒子宽度必须能被 MPCD cell size 整除；用分析半径取整，避免校准半径的小数破坏网格。
    box_width = 2.0 * math.ceil(analysis_radius(config) + 1.0)
    # 每条任务有自己的随机种子。
    seed = int(task["seed"])
    ...
    snapshot.configuration.box = [box_width, box_width, length, 0.0, 0.0, 0.0]
...
def run_with_sampling(sim: Any, n_steps: int, sample_interval: int, sample_gel: bool, radius: float, profile_path: Path, series_path: Path, config: dict[str, Any], run_id: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    while done < n_steps:
        ...
        # 速度剖面只统计后段稳态，避免把流动启动瞬态混进空管验证图。
        profile_start = float(config["output"].get("profile_start_fraction", 0.5))
        if done >= profile_start * n_steps:
...
def run_task(root: Path, config: dict[str, Any], task: dict[str, Any], steps: int | None) -> dict[str, Any]:
    ...
    # 分析半径用于输出和归一化；几何半径用于 HOOMD 边界。
    radius = analysis_radius(config)
    wall_radius = geometry_radius(config)
    # MPCD 温度。
    temperature = float(config["mpcd"]["temperature"])
    # force 是真实体力 g，直接沿 z 方向施加给 MPCD 溶剂。
    force = float(task["force"])
    body_force = force_vector(force)
    ...
    # 圆柱管道几何；outer_radius 使用校准后的几何半径。
    geometry = hoomd.mpcd.geometry.ConcentricCylinders(inner_radius=0.0, outer_radius=wall_radius, no_slip=True)
    ...
    # 虚粒子填充来自旧项目的成熟做法，用来改善无滑移壁面附近的动量交换。
    ...
    integrator = hoomd.mpcd.Integrator(
        ...
        mpcd_particle_sorter=hoomd.mpcd.tune.ParticleSorter(trigger=200),
    )
    ...
    summary = {
        ...
        "geometry_radius": wall_radius,
        "geometry_radius_offset": wall_radius - radius,
...
def main() -> None:
    """提示用户使用具体数据脚本。"""
    raise SystemExit(
        "核心.py 是底层模拟函数文件，不直接运行。\n"
        "单条轨迹数据: python scripts/轨迹.py --structure g3 --flow 1 --seed 101 --gpu 0\n"
        "时间尺度计算和图: python scripts/时间.py"
    )


if __name__ == "__main__":
    # 直接运行 python scripts/核心.py 时只打印提示。
    main()

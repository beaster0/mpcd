...
可以改什么：
- 要改结构大小，改 `config/base.json` 里的 `structure.n_values`。
- 要改每条网格边的链段数，改 `structure.segments_per_edge`。
- 要改初始键长，改 `bead.bond_equilibrium`。
- 要改管半径，改 `pipe.analysis_radius`，因为结构表会计算 `Rg/R`。
- 不建议直接改本文件里的生成公式，除非你要改变网络定义。

输入：
- `config/base.json`：结构、管道和珠子参数。

输出：
- `data/structures/g<n>.json`：每个凝胶结构的珠子坐标和键连接。
- `tables/structures/metrics.csv`：每个结构的珠子数、键数、Rg/R 等几何指标。

运行：
- `/home/zhangxh/students/sjl/miniconda3/envs/A/bin/python scripts/结构.py`
"""
...
def build_grid(n: int, config: dict[str, Any]) -> dict[str, Any]:
    ...
    # 单个键的初始几何长度。
    # FENE-WCA 的真实键势由 核心.py 设置；这里的 bond_equilibrium 只决定初始网格间距。
    ...
    metrics = measure(beads, bonds, radius)
...
if __name__ == "__main__":
    # 只有直接运行 python scripts/结构.py 时才执行 main。

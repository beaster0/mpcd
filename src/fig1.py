...
def save_fig(fig: Figure, outdir: Path) -> Path:
...
def plot_pipe(ax: Axes, radius: float, length: float) -> None:
    ax.axis("off")
    ax.set_aspect("equal")
    panel_label(ax, "A")
...
def plot_structure(ax: Axes, structure: dict[str, Any], title: str) -> None:
...
def plot_networks(fig: Figure, gs_cell: Any, structures: dict[str, dict[str, Any]]) -> None:
...
def plot_metrics(ax: Axes, rows: list[dict[str, str]]) -> None:
    panel_label(ax, "C")
...
def plot_control(ax: Axes, config: dict[str, Any], tau_ref: float) -> None:
    ax.axis("off")
    panel_label(ax, "D")

    g_values = config.get("flow", {}).get("g_values", [0.0])
    g_text = ", ".join(rf"${float(g):g}$" for g in g_values)
    ...
    text = "\n".join(
        [
            ...
            "",
            rf"body force $g_z$: {g_text}",
            "",
            r"$\mathrm{Wi}$ from pure-fluid calibration",
        ]
    )
